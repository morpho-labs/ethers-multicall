...
describe("ethers-multicall", () => {
  ...
  describe("Calls batching", () => {
    ...
    it("should fetch UNI.balanceOf(cUNI) at default block 14_500_000", async () => {
      const multicall = new EthersMulticall(rpcProvider, { defaultBlockTag: 14_500_000 });
      const wrappedUni = multicall.wrap(uni);
    ...
    it("should throw a descriptive Error when querying unknown contract", async () => {
      ...
      expect(wrappedUnknown.symbol()).rejects.toThrow(
        new Error(`0xd6409e50c05879c5B9E091EB01E9Dd776d00A151:symbol() empty return data exception`)
      );
    });
    ...
    it("should query filters", async () => {
      ...
      const events = await wrappedUni.queryFilter(
        wrappedUni.filters.Transfer(),
        14_000_000,
        14_002_000
      );
    ...
    it("should only fail the failing call promise when querying incorrect contract", async () => {
      ...
      expect(wrappedUni.symbol()).resolves.toBe("UNI");
      expect(wrappedUnknown.symbol()).rejects.toThrow(
        "0xd6409e50c05879c5B9E091EB01E9Dd776d00A151:symbol() empty return data exception"
      );
      expect(await wrappedUnknown.symbol().catch(() => "DEFAULT")).toBe("DEFAULT");
    });
    ...
    it("should only fail the failing call promise when querying before multicall deployment", async () => {
      ...
      expect(wrappedUni.symbol({ blockTag: 14_500_000 })).resolves.toBe("UNI");
      expect(wrappedUni.symbol({ blockTag: 14_000_000 })).rejects.toThrow(
      ...
      expect(await wrappedUni.symbol({ blockTag: 14_000_000 }).catch(() => "UNI")).toBe("UNI");
    });
  });
});
